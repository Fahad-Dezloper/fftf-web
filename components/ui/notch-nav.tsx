...
export function NotchNav({
  ...
  return (
    <nav aria-label={ariaLabel} className={["w-fit mx-auto", className].filter(Boolean).join(" ")}>
      <div ref={containerRef} className="relative rounded-lg border-border text-foreground">
        {/* Items */}
        <ul
          role="menubar"
        ...
        >
          {items.map((item, idx) => {
            ...
            return (
              <li key={item.value} role="none" className="">
                <button
                  ...
                  className={[
                    "relative rounded-md px-3 py-2 text-lg font-semibold outline-none transition-colors",
